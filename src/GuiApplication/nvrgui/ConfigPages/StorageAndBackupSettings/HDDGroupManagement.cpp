...
HDDGroupManagement::HDDGroupManagement(QString deviceName, QWidget *parent, DEV_TABLE_INFO_t *devTabInfo)
    : ConfigPageControl(deviceName, parent, MAX_HDD_GROUP_CONTROL, devTabInfo, CNFG_TYPE_DFLT_REF_SAV_BTN)
{
...
HDDGroupManagement::~HDDGroupManagement()
{
...
void HDDGroupManagement::createDefaultComponents()
{
    m_hddGroupHeading = new ElementHeading((SCALE_WIDTH(PAGE_RIGHT_PANEL_WIDTH) - BGTILE_LARGE_SIZE_WIDTH)/2 + SCALE_WIDTH(5),
                                           SCALE_HEIGHT(40),
    ...
    m_hddGroupDropDown = new DropDown(m_hddGroupHeading->x() + SCALE_WIDTH(480),
                                      m_hddGroupHeading->y(),
                                      BGTILE_LARGE_SIZE_WIDTH,
                                      BGTILE_HEIGHT,
                                      HDD_GROUP_DROP_DOWN,
    ...
    m_elementList[HDD_GROUP_DROP_DOWN] = m_hddGroupDropDown;
    ...
    m_volumeSelectHeading = new ElementHeading(m_hddGroupHeading->x(), m_hddGroupHeading->y() +
                                               m_hddGroupHeading->height() + SCALE_HEIGHT(20),
    ...
    m_allVolumeChecklist = new OptionSelectButton(m_volumeSelectHeading->x(),
     ...
     m_elementList[HDD_GROUP_SEL_ALL_VOLUME] = m_allVolumeChecklist;
    ...
    m_cameraSelectHeading = new ElementHeading(m_hddGroupHeading->x() + BGTILE_HDD_GRP_CHECKBOX + SCALE_WIDTH(15), m_hddGroupHeading->y() +
                                               m_hddGroupHeading->height() + SCALE_HEIGHT(20),
                                               BGTILE_HDD_GRP_CHECKBOX,
                                               BGTILE_HEIGHT,
    ...
    m_allCameraChecklist = new OptionSelectButton(m_cameraSelectHeading->x(),
     ...
     m_elementList[HDD_GROUP_SEL_ALL_CAMERA] = m_allCameraChecklist;
    ...
    for(quint8 index = 0 ; index < MAX_CAM_ON_SINGLE_PAGE; index++)
    {
        ...
        m_elementList[HDD_GROUP_CAMERA_LIST + index] = m_cameraListCheckBox[index];
    ...
    m_elementList[HDD_GROUP_CAMERA_PREV_BTN] = m_cameraPrevButton;
    ...
    for(quint8 index = 0; index < MAX_PAGE_NUMBER; index++)
    {
        ...
        m_elementList[(HDD_GROUP_CAMERA_PAGE_NUM_BTN + index)] = m_cameraPageNumberLabel[index];
    ...
    m_elementList[HDD_GROUP_CAMERA_NEXT_BTN] = m_cameraNextButton;
    ...
    m_elementHeadingBitRate = new ElementHeading(m_hddGroupHeading->x() + SCALE_WIDTH(180),
    ...
    m_footnoteLabelBitRate->resize(BGTILE_LARGE_SIZE_WIDTH, SCALE_HEIGHT(50));
    ...
    m_elementHeadingNoDevice = new ElementHeading(m_hddGroupHeading->x() + SCALE_WIDTH(362),
                                               m_hddGroupHeading->y() + SCALE_HEIGHT(300),
                                               BGTILE_LARGE_SIZE_WIDTH,
                                               SCALE_HEIGHT(50),
                                               "",
                                               NO_LAYER,
...
void HDDGroupManagement::createVolumeList()
{
    destroyVolumeList();
    ...
    for(quint8 index = 0 ; index < volListSize; index++)
    {
        ...
        m_elementList[HDD_GROUP_VOLUME_LIST + index] = m_volumeListCheckBox[index];
...
void HDDGroupManagement::destroyVolumeList()
{
    for(quint8 index = 0 ; index < MAX_LOGICAL_VOLUME_POSSIBLE; index++)
    {
        if(IS_VALID_OBJ(m_volumeListCheckBox[index]))
...
void HDDGroupManagement::showHideCtrls(bool isVisible)
{
    m_hddGroupHeading->setVisible(isVisible);
    ...
    showHideCnfgBtn(CNFG_DEFAULT_BTN, isVisible);
    showHideCnfgBtn(CNFG_SAVE_BTN, isVisible);
}

void HDDGroupManagement::fillCameraList()
{
    cameraList.clear();
...
bool HDDGroupManagement::getVolumeNumber(QString volumeName, qint8 &volumeNumber)
{
...
void HDDGroupManagement::getLogicalVolumeStatus()
{
...
void HDDGroupManagement::getConfig()
{
    createPayload(MSG_GET_CFG);
}

void HDDGroupManagement::saveConfig()
{
    if(false == isUserChangeConfig())
    {
        return;
    }
...
void HDDGroupManagement::defaultConfig()
{
    createPayload(MSG_DEF_CFG);
}

void HDDGroupManagement::createPayload(REQ_MSG_ID_e msgType)
{
...
void HDDGroupManagement::processDeviceResponse(DevCommParam *param, QString deviceName)
{
...
void HDDGroupManagement::updateStorageAllocationMask()
{
...
void HDDGroupManagement::showVolumeList()
{
    qint8 volIndex;
    for(quint8 index = 0 ; index < volListSize; index++)
    {
...
void HDDGroupManagement::showCameraList()
{
    quint8 fieldsOnPage;
    ...
    {
        if((m_currentElement == HDD_GROUP_CAMERA_NEXT_BTN) || (m_currentElement == HDD_GROUP_CAMERA_PREV_BTN))
        {
            m_currentElement = HDD_GROUP_CAMERA_NEXT_BTN;
            m_elementList[m_currentElement]->forceActiveFocus ();
        }
    ...
    if(currentPageNum == (totalPages-1))
    {
        if((m_currentElement == HDD_GROUP_CAMERA_NEXT_BTN) || (m_currentElement == HDD_GROUP_CAMERA_PREV_BTN))
        {
            m_currentElement = HDD_GROUP_CAMERA_PREV_BTN;
            m_elementList[m_currentElement]->forceActiveFocus ();
        }
    ...
    for(quint8 index = 0; index < MAX_PAGE_NUMBER; index++)
    {
        ...
        if((index == tempIndex) && ((pageOffset + index) < totalPages))
        {
            ...
            m_cameraPageNumberLabel[index]->setBold (true);
            m_cameraPageNumberLabel[index]->forceActiveFocus ();
            m_currentElement = (HDD_GROUP_CAMERA_PAGE_NUM_BTN + index);
        }
        else
        {
            m_cameraPageNumberLabel[index]->setBackGroundColor (TRANSPARENTKEY_COLOR);
...
void HDDGroupManagement::updatePage(bool isDropDownChng)
{
    qint8 volIndex = 0;
...
bool HDDGroupManagement::isCameraAllocatedToAnyOtherGrp(quint8 camIdx)
{
    for(quint8 grpIdx = 0; grpIdx < STORAGE_ALLOCATION_GROUP_MAX; grpIdx++)
    {
...
bool HDDGroupManagement::isVolumeAllocatedToAnyGrp(quint8 volIdx)
{
    qint8 tVolIndex = 0;
    for(quint8 grpIdx = 0; grpIdx < STORAGE_ALLOCATION_GROUP_MAX; grpIdx++)
    {
...
void HDDGroupManagement::handleInfoPageMessage(int index)
{
...
void HDDGroupManagement::slotDropdownValueChanged(QString idxStr, quint32 index)
{
...
void HDDGroupManagement::slotButtonClick(int index)
{
    switch(index)
    {
...
void HDDGroupManagement::slotOptionButtonClicked(OPTION_STATE_TYPE_e state, int index)
{
...
void HDDGroupManagement::slotPageNumberButtonClick(QString str)
{
    currentPageNum = ((quint8)str.toUInt() - 1);
    showCameraList();
}

bool HDDGroupManagement::isUserChangeConfig()
{
